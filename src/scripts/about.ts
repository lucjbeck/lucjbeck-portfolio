...
export function initAboutAssembly(): void {
  ...
  // ── Run once: assemble the crest, then stay finished (no reset/replay). ────
  const scatterAll = () => {
    for (const p of allPieces) {
      const s = startPos.get(p)!;
      p.el.setAttribute("transform", `translate(${s.x} ${s.y})`);
  ...
  const loop = () => {
    if (finished) { running = false; return; }
    if (!visible)  { running = false; return; }   // pause off-screen; resume later
    tickArm(states[0]);
    tickArm(states[1]);
    if (states[0].done && states[1].done) { finished = true; running = false; return; }
    requestAnimationFrame(loop);
  };
  const ensureRunning = () => {
    if (!running && !finished) { running = true; requestAnimationFrame(loop); }
  };
